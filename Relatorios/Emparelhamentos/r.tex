...
U3 & Escalabilidade & Tempo de parede vs.\ $n$ & 100--10000 & 20 \\
...
W3 & Escalabilidade & Tempo ponderado vs.\ $n$ & 50--2000 & 20 \\
...
O expoente de densidade $\alpha$ toma os valores $\{1{,}0;\, 1{,}25;\, 1{,}5;\, 1{,}75;\, 2{,}0\}$ nos testes de escalabilidade ($U3$, $W3$, $C1$, $C2$) e $\{1{,}0;\, 1{,}5;\, 2{,}0\}$ nos demais. O orçamento de repetições é assimétrico: testes exploratórios usam 10 repetições, escalabilidade usa 20 e os testes de relatório final usam 30, refletindo a necessidade de intervalos de confiança mais estreitos para o ajuste de curvas.
...
O contador \texttt{potential\_violations\_count} foi zero em \textbf{todas} as instâncias de W2, confirmando que a manutenção dos potenciais de Johnson garante $d'_{uv} \ge 0$ para toda aresta residual em todo momento.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{$\alpha = 2{,}0$}
    \end{subfigure}
    \caption{Tempo de parede (mediana) vs.\ $n$ em escala log-log por $\alpha$ (W3). Banda sombreada: mediana a p90; linhas tracejadas: referências teóricas. Para $\alpha \le 1{,}5$, JD é mais rápido; para $\alpha = 2{,}0$, BF é substancialmente mais rápido.}
...
$\alpha$ & Algoritmo & $k_\text{teórico}$ & $k_\text{emp}$ & $R^2$ \\
\midrule
1{,}0  & BellmanFordStrategy          & 3{,}00 & 3{,}02 & 1{,}000 \\
1{,}0  & JohnsonDijkstraStrategy      & 2{,}00 & 2{,}76 & 0{,}999 \\
1{,}25 & BellmanFordStrategy          & 3{,}25 & 3{,}09 & 1{,}000 \\
1{,}25 & JohnsonDijkstraStrategy      & 2{,}25 & 2{,}75 & 1{,}000 \\
1{,}5  & BellmanFordStrategy          & 3{,}50 & 2{,}87 & 1{,}000 \\
1{,}5  & JohnsonDijkstraStrategy      & 2{,}50 & 2{,}65 & 1{,}000 \\
1{,}75 & BellmanFordStrategy          & 3{,}75 & 2{,}91 & 1{,}000 \\
1{,}75 & JohnsonDijkstraStrategy      & 2{,}75 & 2{,}85 & 1{,}000 \\
2{,}0  & BellmanFordStrategy          & 4{,}00 & 2{,}97 & 1{,}000 \\
2{,}0  & JohnsonDijkstraStrategy      & 3{,}00 & 3{,}14 & 1{,}000 \\
\bottomrule
\end{tabular}
\end{table}

Os resultados de W3 (Figuras~\ref{fig:w3-wallclock} e~\ref{fig:w3-heatmap}, Tabela~\ref{tab:w3-exponents}) contêm o achado mais contraintuitivo dos experimentos: para $\alpha = 2{,}0$, JD é \textbf{10 vezes mais lento} que BF para $n = 2000$ (426\,900\,ms vs.\ 40\,942\,ms), apesar da previsão teórica de que JD deveria ser mais rápido. Dois efeitos concorrentes explicam o fenômeno:

\begin{enumerate}
    \item \textbf{BF com terminação antecipada:} o expoente empírico de BF é $k_\text{emp} \approx 2{,}9$--$3{,}0$ para todos os $\alpha$, \emph{muito abaixo} do teórico $2 + \alpha = 4{,}0$ para $\alpha = 2{,}0$. A flag \texttt{relaxed} elimina os passos de relaxação desnecessários, e o acesso sequencial à matriz de pesos tem excelente localidade de cache.

    \item \textbf{Overhead do heap de JD:} para $\alpha = 2{,}0$, há $n^2$ arestas. Cada chamada do Dijkstra pode inserir $O(n^2)$ entradas na fila de prioridade. As operações de heap têm alto custo constante e péssima localidade de cache comparadas às relaxações simples de BF. O expoente empírico de JD ($k_\text{emp} = 3{,}14$) supera o de BF ($k_\text{emp} = 2{,}97$) para $\alpha = 2{,}0$.
\end{enumerate}

Para $\alpha = 1{,}0$, JD é $\sim 6\times$ mais rápido que BF a $n = 2000$, e para $\alpha = 1{,}5$, JD é $\sim 3\times$ mais rápido. A vantagem de JD é máxima em grafos esparsos, onde a fila de prioridade é pequena e o custo de BF por passo é dominado pelo percurso de todos os $n$ vértices.
...
O teste W4 compara os três regimes de peso (Figura~\ref{fig:w4}): misto $[-n^2,\, n^2]$, totalmente negativo $[-n^2,\, 0]$ e totalmente positivo $[0,\, n^2]$. Em \textbf{todas as combinações} de $(n, \alpha, \text{regime})$, BF e JD produziram valores de emparelhamento \emph{idênticos} (diferença absoluta $< 10^{-6}$). O contador \texttt{potential\_violations\_count} permaneceu \textbf{zero em todos os regimes}, incluindo o totalmente negativo --- onde o potencial inicial $p_v = -W$ é mais tensionado.

Um resultado curioso é que os valores de emparelhamento para o regime misto e o regime negativo foram \textbf{idênticos} para $n = 500$ e $n = 1000$. Isso indica que, para os grafos gerados, o emparelhamento perfeito de máxima cardinalidade no regime misto escolhe automaticamente arestas com pesos negativos --- a estrutura do grafo esparso não oferece alternativas de alto peso para todos os $n$ vértices da partição.
...
\textbf{Implicação prática:} Para emparelhamentos não ponderados, recomenda-se HK para qualquer grafo com $\alpha \ge 1{,}5$; para $\alpha = 1{,}0$, Simple é preferível até $n = 10000$. Para emparelhamentos ponderados com grafos esparsos a médios ($\alpha \le 1{,}5$), JD é a escolha correta. Para grafos densos ($\alpha \approx 2{,}0$), BF com terminação antecipada apresenta desempenho inesperadamente competitivo, e JD só deve ser preferido se o $n$ for suficientemente grande para amortizar o overhead da heap.
