...
%==================================================
\section{Introdução}
Este relatório descreve a avaliação empírica de seis algoritmos de fluxo máximo $s-t$ da família Ford-Fulkerson. O objetivo é verificar a complexidade experimental, comparando o desempenho real com os limites teóricos pessimistas. 
...
Esta seção apresenta os gráficos de desempenho e a análise quantitativa das três métricas
de defeito, $r$, $\overline{s}$ e $\overline{t}$, confrontando os valores experimentais
com os limites teóricos pessimistas (ver Tabela~\ref{tab:limites-teoricos}). Os dados
numéricos completos encontram-se no Apêndice~\ref{apx:tabelas}.
...
ainda implica $r = 0{,}0046$. A Tabela~\ref{tab:dinitz-fases} resume a evolução de $r$ com $n$ para Dinitz:

\begin{table}[H]
\centering
\caption{Dinitz: $F$ real vs.\ $\overline{F} = n$ por família e escala.}
\label{tab:dinitz-fases}
\begin{tabular}{lrrrr}
\toprule
...
Os gráficos a seguir ilustram o comportamento de $r$ em função da densidade e da escala
para ambas as famílias de grafos.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images/r_density_DoubleExpLine.png}
    \caption{Razão de Fases ($r$) vs Densidade (DoubleExpLine): as fases reais são uma fração mínima do limite teórico pessimista, tendendo a zero com o aumento de $m$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images/r_scaling_DoubleExpLine.png}
    \caption{Razão de Fases ($r$) vs Escala (DoubleExpLine): o limite teórico cresce explosivamente enquanto o número real de fases cresce muito lentamente.}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Razão de Fases ($r$) vs Escala (BasicLine): o ``defeito'' das fórmulas de complexidade pessimista aumenta proporcionalmente ao tamanho da rede.}
\end{figure}
...
Os gráficos abaixo mostram o defeito de arcos $\overline{t}$ em função da densidade
e da escala.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images/t_bar_density_DoubleExpLine.png}
    \caption{Defeito de Arcos ($\overline{t}$) vs Densidade (DoubleExpLine): EK (BFS) toca quase todos os arcos ($\approx 1{,}0$), enquanto FF e RDFS (DFS) tocam muito menos.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images/t_bar_scaling_DoubleExpLine.png}
    \caption{Defeito de Arcos ($\overline{t}$) vs Escala (DoubleExpLine): Dinitz apresenta $\overline{t} > 1$ pois realiza múltiplas travessias por fase para construir o grafo de níveis e o fluxo bloqueante.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images/t_bar_density_BasicLine.png}
    \caption{Defeito de Arcos ($\overline{t}$) vs Densidade (BasicLine): a consistência comportamental dos algoritmos se mantém; EK explorando amplamente e FF/RDFS explorando pontualmente.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images/t_bar_scaling_BasicLine.png}
    \caption{Defeito de Arcos ($\overline{t}$) vs Escala (BasicLine): a eficiência de busca por fase permanece estável conforme o número de vértices aumenta.}
\end{figure}
...
Os gráficos de tempo de execução confirmam e complementam a análise das métricas de
defeito, revelando o custo real de cada estratégia nas duas famílias de grafos.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images/time_density_DoubleExpLine.png}
    \caption{Tempo de Execução vs Densidade (DoubleExpLine): Dinitz forma a curva mais baixa e estável, enquanto o Fattest Path cresce mais rápido devido ao custo do Heap.}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Tempo de Execução vs Densidade (BasicLine): a sobrecarga residual de estruturas de dados como filas de prioridade (FP) é visível mesmo em grafos mais simples.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images/time_scaling_BasicLine.png}
    \caption{Tempo de Execução vs Escala (BasicLine): Dinitz e Edmonds-Karp apresentam o crescimento assintótico real mais favorável.}
\end{figure}
...
%==================================================
% APÊNDICE: TABELAS DE RESULTADOS NUMÉRICOS
%==================================================
\appendix
\chapter{Tabelas de Resultados Numéricos}
\label{apx:tabelas}
...
% --------------------------------------------------
\section{DoubleExpLine -- Experimento A (Densidade, $n$ fixo)}
% --------------------------------------------------
...
% --------------------------------------------------
\section{DoubleExpLine -- Experimento B (Escala, densidade fixa)}
% --------------------------------------------------
...
% --------------------------------------------------
\section{BasicLine -- Experimento A (Densidade, $n$ fixo)}
% --------------------------------------------------
...
% --------------------------------------------------
\section{BasicLine -- Experimento B (Escala, densidade fixa)}
% --------------------------------------------------
